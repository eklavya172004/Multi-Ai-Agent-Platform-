  ...
  leading-relaxed
        ${
          isUser
            ...
            : "bg-white/[0.06] backdrop-blur-md border border-white/[0.08] text-slate-200 rounded-tl-sm shadow-lg shadow-black/20"
        }`}
      >
        {images.length > 0 && (
  ...
  <div className="flex flex-col gap-2 mt-4">
    {files.map((file, i) => (
      <a
        ...
        rel="noreferrer"
        download={file.name}
        className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition text-sm text-slate-200"
      >
        📄
        <span className="truncate flex-1">{file.name}</span>
        <FiExternalLink size={14} className="opacity-70 shrink-0" />
      </a>
    ))}
  </div>
)}
